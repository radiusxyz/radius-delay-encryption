...
impl<F: PrimeField> BigIntInstructions<F> for BigIntChip<F> {
    ...
    fn refresh(
        ...
        for refreshed_limb in refreshed_limbs.iter().take(num_limbs_fresh) {
            let limb_val = refreshed_limb.value().map(|f| *f);
    ...
    fn add_mod(
        ...
        for res_limb in res_limbs.iter().take(num_limbs).skip(n.num_limbs()) {
            self.main_gate()
                .assert_zero(ctx, &res_limb.assigned_val())?;
        }
