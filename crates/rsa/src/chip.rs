...
use big_integer::{AssignedInteger, BigIntChip, BigIntConfig, BigIntInstructions};
use ff::PrimeField;
use halo2wrong::halo2::plonk::Error;
use maingate::{
    big_to_fe, fe_to_big, AssignedValue, MainGate, MainGateInstructions, RangeChip,
    RangeInstructions, RegionCtx,
};
use num_bigint::BigUint;

use crate::{
    AssignedRSAPubE, AssignedRSAPublicKey, AssignedRSASignature, Fresh, RSAInstructions, RSAPubE,
    RSAPublicKey, RSASignature,
};
...
impl<F: PrimeField> RSAChip<F> {
    ...
    ///
    /// # Arguments
    /// * limb_count - the default number of limbs of [`Fresh`] integers.
    ///
    /// # Return values
    ...
    pub fn compute_range_lens(limb_count: usize) -> (Vec<usize>, Vec<usize>) {
        let (mut composition_bit_lens, overflow_bit_lens) =
            BigIntChip::<F>::compute_range_lens(Self::LIMB_WIDTH, limb_count);
...
mod tests {
    ...
    use maingate::{
        big_to_fe, decompose_big, fe_to_big, AssignedValue, MainGate, MainGateInstructions,
        RangeChip, RangeConfig, RangeInstructions, RegionCtx,
    };
        ...
        fn synthesize(
            ...
            layouter.assign_region(
                ...
                |region| {
                    ...
                    let x_limbs = decompose_big::<F>(self.x.clone(), limb_count, limb_width);
        ...
        fn synthesize(
            ...
            layouter.assign_region(
                ...
                |region| {
                    ...
                    let n_limbs = decompose_big::<F>(n_big.clone(), limb_count, limb_width);
                    let n_unassigned = UnassignedInteger::from(n_limbs);
                    ...
                    let sign_limbs = decompose_big::<F>(sign_big.clone(), limb_count, limb_width);
